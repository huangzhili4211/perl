...
if($help){
    die"usage:perl venn_result.pl [cp1:cp2;cp3:cp4]\n";
}
my @cp = split/;/,$cp;
my $cp_a = $cp[0];
my $cp_b = $cp[1];
my $cp_c = $cp[2];
my $cp_d = $cp[3];
open(FH_A,"<$cp_a.txt") or die "$cp_a:$!\n";
...
my $out_head = "Identified by\tTotal\tAccession\tNames\tDiff_state\tratio\tratio_PVal\tGO_function(Biological_Process)\tGO_function(Cellular_Component)\tGO_function(Molecular_Function)\tCOG_function\tKegg_function\tGO_enrichment(Biological_Process)\tGO_enrichment(Cellular_Component)\tGO_enrichment(Molecular_Function)\tKegg_enrichment";
my @out_head = split/\t/,$out_head;
my $workbook = Excel::Writer::XLSX->new("venn_result.xlsx");
my $worksheet = $workbook->add_worksheet("venn_result");
my %font_head = (
...
foreach(@venn_group){
	...
	foreach(@cp){
	    ...
	    my @out_items = split/\t/,$out_items;
	    if($line2==0){
		$worksheet->write($line2,0,\@out_head,$headFont);
		$line2++;
		$worksheet->write($line2,0,\@out_items,$bodyFont);
		$line2++;
	    }elsif($id_num == $id_n && $line == $cp_n){
		$worksheet->write($line2,0,\@out_items,$bodyFont);
		$line2+=2;
		#print OUT "$out_items\n";
	    }else{
